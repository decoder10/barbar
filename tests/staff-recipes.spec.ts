...
test('staff edits existing recipes and sees stock pictures and shortages without money or stock controls', async ({
  ...
  await card.click();
  await expect(page.getByLabel('Количество ингредиента 1, мл')).toHaveValue('50');
  await expect(page.getByRole('dialog')).toContainText('Нет в наличии');
  await page.getByLabel('Количество ингредиента 1, мл').fill('60');
  ...
  await page.getByLabel('Количество ингредиента 3, г').fill('5');
  await page.getByLabel('Как приготовить').fill('Перемешать со льдом.');
  ...
  await expect(page.getByLabel('Количество ингредиента 1, мл')).toHaveValue('60');
  await expect(page.getByLabel('Количество ингредиента 3, г')).toHaveValue('5');
