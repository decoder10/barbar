# Barbar Cafe — подготовка выпуска уведомлений, 23 сентября 2026

Статус: локальная подготовка; коммит, push и публикация на этом этапе не выполнялись. Назначение отдельного release-этапа: `origin` (`decoder10/barbar`), ветка `barbar`, production `https://barbar-cafe.netlify.app`. Пользователь запросил деплой подготовленных изменений.
